...
//Derek@LTN-SW1-SYSTEM : Create sysfs for force power off from JNI:2011-09-01
...
void power_down_timeout(struct work_struct *_work);

 
struct power_down_devdata {
      struct delayed_work power_down_work;
};
static ssize_t store_power_down(struct device *d,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct power_down_devdata *devdata = dev_get_drvdata(d);
	char *after;
	...
	{
		printk("[LTN] %s(%d)\n", __func__, __LINE__);
		schedule_delayed_work(&devdata->power_down_work,
...
static DEVICE_ATTR(power_down, S_IRUGO|S_IWUSR, store_power_down, store_power_down);
...
//Derek@LTN-SW1-SYSTEM : Requested by Juno.Bae for DFT Item:2011-08-02
//We will enable this feature after implementing application 
#if 0 //CONFIG_LTN_UPLOAD

struct device *upload_dev;

static ssize_t goto_upload(void)
{
		printk("[Derek] %s(%d)\n", __func__, __LINE__);
		panic("Go to Upload by keystring");
}


static DEVICE_ATTR(call_upload, S_IRUGO|S_IWUSR, goto_upload, goto_upload);
#endif
/****************************************************************************
*
...
#if 0
	int r=0;
	struct power_down_devdata *devdata;
#endif
...
//Derek@LTN-SW1-SYSTEM : Create sysfs for force power off from JNI:2011-09-01
	...
	if (devdata == NULL) {
	    printk("Invalid platform data of power down\n");	
		r = -EINVAL;
	}
	INIT_DELAYED_WORK(&devdata->power_down_work, power_down_timeout);
#endif 	
...
//We will enable this feature after implementing application 
	...
	upload_dev = device_create(kril_class, NULL, 0, NULL, "upload");

	if(IS_ERR(upload_dev))
		pr_err("Failed to create device(upload)!\n");
	if(device_create_file(upload_dev, &dev_attr_call_upload) < 0)
		pr_err("Failed to create device file(%s)!\n", dev_attr_call_upload.attr.name);
...
//Derek@LTN-SW1-SYSTEM : Create sysfs for force power off from JNI:2011-09-01
	...
	dev_set_drvdata(pm_dev, devdata);
