					...
					const char	**country_name )
{
	...
	KRIL_DEBUG(DBG_ERROR, "GetSamsungLatinPLMNname ::  deci MCC=%d, MNC=%d \n", plmn_mcc_deci, plmn_mnc_deci);
		...
		//LOGD("plmn='%s', plmnId=%d", plmn, plmnId);
      	...
      	if((strncmp(sales_code,"ZVV",3)==0)||(strncmp(sales_code,"ZVO",3)==0)) //denis.b - PLMN list
		{
		...
		else  if(strncmp(sales_code,"COM",3)==0) //COMCEL -9 //2010.11. Julian requested
		{
			...
			else if((plmn_mcc_deci == 740)&&(plmn_mnc_deci == 01))
			{
				if (long_name != NULL)	
					memcpy(long_name,"PORTA",strlen("PORTA"));
				if (short_name != NULL)
					memcpy(short_name,"PORTA",strlen("PORTA"));
					found = TRUE;
			}
		...
		else if(strncmp(sales_code,"TCE",3)==0) //TELCEL-21
		{
				...
				((plmn_mcc_deci == 330)&&(plmn_mnc_deci == 110)))	
			{
				...
				if (short_name != NULL)
					memcpy(short_name,"PRClaro",strlen("PRClaro"));
					found = TRUE;

			}
			else if(((plmn_mcc_deci == 334)&&(plmn_mnc_deci == 02))||
				((plmn_mcc_deci == 334)&&(plmn_mnc_deci == 20))||
				((plmn_mcc_deci == 334)&&(plmn_mnc_deci == 020)))
			{
				if (long_name != NULL)	
					memcpy(long_name,"TELCEL",strlen("TELCEL"));
				if (short_name != NULL)
			...
			else if((plmn_mcc_deci == 370)&&(plmn_mnc_deci == 02))
			{
				if (long_name != NULL)	
			...
			else if((plmn_mcc_deci == 704)&&(plmn_mnc_deci == 01))
			{
				if (long_name != NULL)	
			...
			else if((plmn_mcc_deci == 706)&&(plmn_mnc_deci == 01))
			{
				if (long_name != NULL)	
			...
			else if(((plmn_mcc_deci == 708)&&(plmn_mnc_deci == 00))||
				((plmn_mcc_deci == 708)&&(plmn_mnc_deci == 01))||
				((plmn_mcc_deci == 708)&&(plmn_mnc_deci == 001)))
			{
				if (long_name != NULL)	
				...
				if (short_name != NULL)
					memcpy(short_name,"Claro HND",strlen("Claro HND"));
					found = TRUE;
			...
			else if((plmn_mcc_deci == 712)&&(plmn_mnc_deci == 03))
			{
				if (long_name != NULL)	
			...
			else if((plmn_mcc_deci == 714)&&(plmn_mnc_deci == 03))
			{
				if (long_name != NULL)	
			...
			else if((plmn_mcc_deci == 716)&&(plmn_mnc_deci == 10))
			{
				...
				if (short_name != NULL)
					memcpy(short_name,"Claro PER",strlen("Claro PER"));
					found = TRUE;
			...
			else if((plmn_mcc_deci == 730)&&(plmn_mnc_deci == 03))
			{
				if (long_name != NULL)	
			...
			else if((plmn_mcc_deci == 732)&&(plmn_mnc_deci == 101))
			{
				...
				if (short_name != NULL)
					memcpy(short_name,"COMCEL",strlen("COMCEL"));
					found = TRUE;					
			}
			else if((plmn_mcc_deci == 740)&&(plmn_mnc_deci == 01))
			{
			...
			else if((plmn_mcc_deci == 744)&&(plmn_mnc_deci == 02))
			{
				if (long_name != NULL)	
    ...
    switch(pdata->handler_state)
    {
        ...
        case BCM_RESPCAPI2Cmd:
        {
			...
			if(strlen(sales_code) == 0)
				...
				KRIL_DEBUG(DBG_INFO, "longNametype:%d shortname type %d Mcc/Mnc:%d /%d Rat %d\n", nameResult->long_name.nameType,nameResult->short_name.nameType,v_current_mcc,v_current_mnc,gRegInfo.netInfo.rat);
#endif
			     ...
			     {
			        if( ((v_current_mcc == 0x724)&&((v_current_mnc==0x10)||(v_current_mnc==0x11)||(v_current_mnc==0x06)||(v_current_mnc==0x23))&&(gRegInfo.netInfo.rat != RAT_UMTS))||
					((v_current_mcc == 0x724)&&((v_current_mnc==0x02)||(v_current_mnc==0x03)||(v_current_mnc==0x04))&&(gRegInfo.netInfo.rat != RAT_UMTS)))
			        	{
			        	coding = (nameResult->long_name.coding==0x00)?FALSE:TRUE;
						if(!GetSamsungLatinPLMNname(coding, v_current_mcc, v_current_mnc, rdata->longname, NULL, NULL))
					GetSamsungPLMNname(coding, v_current_mcc, v_current_mnc, rdata->longname, NULL, NULL);
    ...
    switch(pdata->handler_state)
    {
		...
		// }
		   ...
		   if( (rsp->searched_plmn[i].nonUcs2Name.longName.nameType== PLMN_NAME_TYPE_LKUP_TABLE) || (rsp->searched_plmn[i].nonUcs2Name.longName.nameType== PLMN_NAME_TYPE_INVALID) )
		   {
				...
				#if defined (CONFIG_LTN_COMMON) //fabio.p 30-aug-2011 - P110823-0127/P110823-0117 - Customize PLMN list in manual search 
		   ...
		   if( (rsp->searched_plmn[i].nonUcs2Name.shortName.nameType == PLMN_NAME_TYPE_LKUP_TABLE) || (rsp->searched_plmn[i].nonUcs2Name.shortName.nameType == PLMN_NAME_TYPE_INVALID) )
				...
				GetSamsungPLMNname(coding, v_current_mcc, v_current_mnc, NULL,  rdata->available_plmn[rdata->num_of_plmn].shortname, NULL);
                        ...
                        else if(rsp->searched_plmn[i].nonUcs2Name.shortName.nameType == PLMN_NAME_TYPE_NITZ)
			     {
					...
					GetSamsungPLMNname(coding, v_current_mcc, v_current_mnc, rdata->available_plmn[rdata->num_of_plmn].shortname, NULL, NULL);
                }			   	
