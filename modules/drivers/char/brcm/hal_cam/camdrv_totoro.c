...
#include "camacq_sr200pc10-ltn.h"
extern int camera_antibanding_get();
#else
#include "camacq_sr200pc10.h"//BYKIM_DTP
#endif //latin_cam : support anti-banding 2011.07.12 hmin84.park 
...
#elif defined(CONFIG_TOTORO_L_LTN_COMMON)
	"GT-S5360L" //target name
#elif defined(CONFIG_TOTORO_B_LTN_COMMON)
	"GT-S5360B" //target name
#elif defined(CONFIG_TARGET_LOCALE_SEA_THL)
	"GT-S5360B" //target name
#else
	"GT-S5360" //target name
#endif //latin_cam : support Exif model name 2011.07.07 hmin84.park 
};
...
int getsizeRow2D(const U8 pp[][2])
{
	...
	printk(KERN_DEBUG"[LTN] row_size is [%d]\n", indexRow);
...
{\
	...
	errTableIndex--;\
	if((getsizeRow2D(TABLE_NAME##_60hz) == rowSize) && (rowSize > 0))\
		{memcpy (TABLE_NAME, TABLE_NAME##_60hz, rowSize*sizeof(U8)*2);\
		printk(KERN_DEBUG"[LTN] memcpy Done! rowSize = [%d]\n", rowSize);}\
	else return errTableIndex;\
}
...
/*
...
#if defined(LTN_CAM_DEBUG)	//latin_mm:jyc for debug
	{
		...
		printk(KERN_DEBUG"[LTN] Display setting table ###\n");
		
		//printk(KERN_DEBUG"[LTN] reg_main_scene_auto : [%d]\n", reg_main_scene_auto); 
		//printk(KERN_DEBUG"[LTN] reg_main_scene_auto : [%d]\n", &reg_main_scene_auto[0][0]);

		count = getsizeRow2D(reg_main_scene_auto_60hz);

		for( ii = 0 ; ii < count ; ii++)
		{
			printk(KERN_DEBUG"{0x%02x, 0x%02x},\n", reg_main_scene_auto[ii][0], reg_main_scene_auto[ii][1]);
		}	
	}
#endif
*/
...
{
#define CAMERA_ANTIBANDING_PATH "/sys/devices/virtual/camera/camera/anti-banding" 

	int result = 0;
	int value = 0;
/*
	value = camera_antibanding_get();
	printk(KERN_DEBUG"[LTN] antibanding value is %d\n", value);

	if(CAM_ANTIBANDING_60HZ == value) { 
		result = CAMDRV_SetTableFor60hzAntibanding();
		if(result < 0)
		{
			printk(KERN_ERR"[LTN] CheckTableSize fail! Error_table is [%d]\n", result);
			return HAL_CAM_ERROR_INTERNAL_ERROR;
		}
	}
*/
...
#endif //latin_cam : support anti-banding 2011.07.08 hmin84.park 
...
HAL_CAM_Result_en_t CAMDRV_Wakeup(CamSensorSelect_t sensor)
{
	...
	if(result != HAL_CAM_SUCCESS)
	{
		printk(KERN_ERR"[LTN] Totoro_sensor_set_60hz_antibanding fail! Error = %d \n", result);
		return result;
	}
#endif //latin_cam : support anti-banding 2011.07.08 hmin84.park
...
#endif //latin_cam : for latin exif model name 2011.08.12 hmin84.park
