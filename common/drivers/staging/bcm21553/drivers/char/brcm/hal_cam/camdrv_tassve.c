...
CAM_Sensor_Supported_Params_t CamPrimaryDefault_st =
{
	...
	"SAMSUNG",
#if defined(CONFIG_TOTORO_LTN_COMMON)
	"GT-S5367" //target name
#else
	"TASSVE" //target name
#endif //ltn_cam : support exif model name hmin84.park 2011.12.12
};
					...
					CamSensorSelect_t sensor)
{
	...
	gv_Nightshot_mode =FALSE;
	    ...
	    pstSensor->m_pstAPIs->WriteDirectSensorData( pstSensor, CAMACQ_SENSORDATA_SCENE_NONE); //ltn_cam : added reset code before scene mode setting - merge CL 764632 hmin84.park
        }
	...
	break;
        ...
        //ltn_cam : add SCN Sports in TotoroTV 3M Cam hmin84.park 2011.12.22
...
UInt8  CAMDRV_CheckEXP(UInt8 mode)
{
	...
	if(mode == CAM_SHUTTER_SPEED)/*shutter speed */
	{
		...
		gv_checkEXPtime = ((ReadShutterSpeed2[0]<<24)|(ReadShutterSpeed2[1]<<16)|(ReadShutterSpeed1[0]<<8)|ReadShutterSpeed1[1])/40; //default not 1msec but 0.1msec
		printk(KERN_INFO"gv_checkEXPtime = %d \r\n",gv_checkEXPtime);
		gv_checkEXPtime = 10000 / gv_checkEXPtime ; // ex : 0.5 msec = 1/2000 sec 
		printk(KERN_INFO"[TotoroTV] ExpValue = [1/%d] \r\n",gv_checkEXPtime);

		if(gv_checkEXPtime <= 0)
		{
			gv_checkEXPtime =1;
			printk(KERN_ERR"gv_checkEXPtime 2: %d \r\n",gv_checkEXPtime);
		}
...
#if 1 // ltn_cam : fixed ISO Rating for Partn_Indoor, Beach_Snow, Firework hmin84.park 2011.12.26
...
HAL_CAM_Result_en_t CAMDRV_GetSensorValuesForEXIF( CAM_Sensor_Values_For_Exif_t *exif_parm,CamSensorSelect_t sensor)
{
    ...
    CAMDRV_CheckISO();    
    sprintf(aString, "%d,", gv_isoSpeedRatings); //ltn_cam : fix Iso ratings hmin84.park 2011.12.23
    strcpy(exif_parm->isoSpeedRating,aString);

    strcpy(exif_parm->FNumber,(char *)"26/10" ); //ltn_cam : change Exif info hmin84.park 2011.12.26
    strcpy(exif_parm->maxLensAperture,(char *)"2757/1000" ); //ltn_cam : F2.6 == 2757/1000 hmin84.park 2012.01.11 
    strcpy(exif_parm->lensFocalLength,(char *)"279/100" );  //ltn_cam : change Exif info hmin84.park 2011.12.26

    strcpy(exif_parm->exposureProgram,"3" );
    strcpy(exif_parm->colorSpaceInfo,"1");

    strcpy(exif_parm->softwareUsed,"S5367VJKL1");  //ltn_cam : fix Exif info hmin84.park 2011.12.21

    strcpy(exif_parm->shutterSpeed,"" );//NOT_USED 
    strcpy(exif_parm->aperture,"" );//NOT_USED 
    strcpy(exif_parm->brightness,"" );//NOT_USED 

    //ltn_cam : fix Exif info hmin84.park 2011.12.21
