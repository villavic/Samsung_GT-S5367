...
#define FCI_RINGBUFFER_PEEK(rbuf,offs) (rbuf)->data[((rbuf)->pread+(offs))%(rbuf)->size]

#define FCI_RINGBUFFER_SKIP(rbuf,num) (rbuf)->pread=((rbuf)->pread+(num))%(rbuf)->size

extern ssize_t fci_ringbuffer_read_user(struct fci_ringbuffer *rbuf, u8 __user *buf, size_t len);

extern void fci_ringbuffer_read(struct fci_ringbuffer *rbuf, u8 *buf, size_t len);


#define FCI_RINGBUFFER_WRITE_BYTE(rbuf,byte) { (rbuf)->data[(rbuf)->pwrite]=(byte); \
					       (rbuf)->pwrite=((rbuf)->pwrite+1)%(rbuf)->size; }

extern ssize_t fci_ringbuffer_write(struct fci_ringbuffer *rbuf, const u8 *buf, size_t len);

extern ssize_t fci_ringbuffer_pkt_write(struct fci_ringbuffer *rbuf, u8* buf, size_t len);

extern ssize_t fci_ringbuffer_pkt_read_user(struct fci_ringbuffer *rbuf, size_t idx, int offset, u8 __user *buf, size_t len);

extern ssize_t fci_ringbuffer_pkt_read(struct fci_ringbuffer *rbuf, size_t idx, int offset, u8 *buf, size_t len);
