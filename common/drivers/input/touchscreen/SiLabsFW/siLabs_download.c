...
#include <asm/gpio.h>
#include <asm/io.h>
#include "siLabs_download.h"
#if defined(CONFIG_LTN_DTV)
#include "SILABS_S5367_SMAC002_BL006_APP020.h"
#else
#include "SILABS_S5368_SMAC002_BL006_APP014.h"
#endif
...
int WriteBytes (unsigned int addr, unsigned int num)
{
   ...
   for(i=0;i<num;i++) SMB_DATA_OUT[i+9] = Binary[addr+i]; // temporary code data
...
int GetPageCRC (unsigned int addr)
{
   ...
   for (i = 0; i < 0x200/0x20 ; i++) // Page size = 0x200
   {
        for (j = 0; j < 32 ; j++)     
         Update_CRC (j); // temporary code data
   }
...
int Firmware_Download ()
{
   ...
   if(res == 0) 
   {
        printk("[TSP] %s, %d : fail to GetInfo\n", __func__, __LINE__ );
        return 0;
   }

   res = EraseFlashPage ( 0x7A00 );

  if(res == 0)
  {
     printk("[TSP] %s, %d : fail to EraseFlashPage\n", __func__, __LINE__ );
     return 0;
  }
   ...
   for(addr = APP_START_ADDR ; addr <= APP_END_ADDR ; addr+=32)
   {
       ...
       {
           printk("[TSP] %s, %d : fail to FW update\n", __func__, __LINE__ );
           return 0;
       }
       ...
       if (0)// (((addr + 32) % 0x200) == 0)
       {
           ...
           {
               printk("[TSP] %s, %d : fail to get CRC\n", __func__, __LINE__ );
               return 0;
           }
       }
   }
   ...
   if(res == 0)
    {
        printk("[TSP] %s, %d : fail to resetMCU\n", __func__, __LINE__ );
        return 0;
   }
   
    printk("[TSP] %s, %d : success to update FW\n", __func__, __LINE__ );
