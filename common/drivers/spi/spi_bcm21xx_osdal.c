...
static int bcm21xx_spi_read(struct driver_data *drv_data)
{
	...
	int tx_len=0, rx_len=0, read_length=0;
#endif
	...
	if (!(spi->mode & SPI_LOOP)) {
		while ((readw(drv_data->ioaddr + SPI_SSPSR) & SPI_SSPSR_TNF)
			   && (tx_len > 0))
		{
			if(tx_len > FIFO_DEPTH)
			{
				read_length = FIFO_DEPTH;	
			}
			else
			{
				read_length = tx_len;	
...
#ifdef CONFIG_LTN_DTV
	writeb(0x1, drv_data->ioaddr +(0x1000));  // SPI_CRCCR : CRC is enabled for using register bit FSSNEW
#endif
...
#ifdef CONFIG_LTN_DTV
	writel(readl(drv_data->ioaddr + SPI_SPIPADPULL) | (0x00000003),
		   drv_data->ioaddr + SPI_SPIPADPULL);
#else
...
static int bcm21xx_spi_probe(struct platform_device *pdev)
{
	...
	/* configure multiplex pins for SPI */
