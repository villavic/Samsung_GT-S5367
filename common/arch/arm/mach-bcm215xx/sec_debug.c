...
#ifdef CONFIG_LTN_TCE_DEBUGPACK   //[latin_protocol] wkj 111103 block debugging packet when debug level low&mid
int debug_level_LTN=1;
EXPORT_SYMBOL(debug_level_LTN);
#endif
...
static int sec_debug_set_debug_level(char *str)
{
    ...
    #ifdef CONFIG_LTN_TCE_DEBUGPACK   //[latin_protocol] wkj 111103 block debugging packet when debug level low&mid
    debug_level_LTN = debug_level;
	char *p = debug_level_LTN;
		printk("[Debug LEVEL LTN=%d\n", debug_level_LTN);
	#endif
	return 0;
}
