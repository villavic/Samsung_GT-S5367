...
#define LTN_DTV_TSP_MODIFICATION              1
#define GPIO_GPS_EN_NUM            10
#define GPIO_GPS_RST_N            23
#if !defined(LTN_DTV_TSP_MODIFICATION)
#define SPI_CS_GPIO_ENABLE         1
#define SPI_CS_GPIO_NUM            35
#endif
#endif
...
#if defined(CONFIG_TOUCHSCREEN_MMS128_REV04) || defined(CONFIG_TOUCHSCREEN_MMS128) ||  defined(CONFIG_TOUCHSCREEN_F760)  || (defined(CONFIG_LTN_DTV) && defined (CONFIG_TOUCHSCREEN_SILICONLABS_I2C))
...
#if defined(CONFIG_LTN_DTV) && defined(CONFIG_BCM_CAM_S5K5CCGX)
...
#if defined(CONFIG_LTN_DTV) && defined(LTN_DTV_TSP_MODIFICATION) 
#define TS_GPIO_IRQ	9
#define TSP_INT 9
#define TSP_SDA 6
#define TSP_SCL 3
#else
...
#endif
#if defined(CONFIG_LTN_DTV) && defined (CONFIG_TOUCHSCREEN_SILICONLABS_I2C)
	{
		I2C_BOARD_INFO("siliconLabs-ts", 0x20),
		.irq = GPIO_TO_IRQ(TSP_INT),
	},
#endif
...
#if defined(CONFIG_LTN_DTV) && defined(SPI_CS_GPIO_ENABLE)
	gpio_request(SPI_CS_GPIO_NUM, "spi_cs");
	gpio_direction_output(SPI_CS_GPIO_NUM, 1);
	gpio_set_value(SPI_CS_GPIO_NUM, cs);
	gpio_free(SPI_CS_GPIO_NUM);	
#endif
...
static struct bcm21xx_spi_platform_data bcm21xx_spi_info = {
	...
	.cs_line = 1,
#if defined(CONFIG_LTN_DTV)
	.mode = 1,		/* Configure for mode, master=1, slave=0*/
#else
...
#if defined(CONFIG_LTN_DTV)
	 {
			 .modalias = "fc8101_spi",	/* use spidev generic driver */
			 .max_speed_hz = 8000000,	/* use max speed */
			 .bus_num = 0,		/* framework bus number */
			 .chip_select = 0,	/* for each slave */
			 .platform_data = NULL, /* no spi_driver specific */
			 .controller_data = &bcm21xx_spi_info,
			 .irq = GPIO_TO_IRQ(12), /* IRQ for this device */
...
int board_sysconfig(uint32_t module, uint32_t op)
{
	...
	case SYSCFG_SPI1:
		...
		if (op == SYSCFG_INIT) {
			 writel((readl(ADDR_SYSCFG_IOCR0) 
				& ~(SYSCFG_IOCR0_GPIO53_GPEN8_MUX | SYSCFG_IOCR0_GPEN9_SPI_GPIO54_L_MUX | SYSCFG_IOCR0_GPIO55_GPEN10_MUX(1) | SYSCFG_IOCR0_GPEN11_SPI_GPIO56_L_MUX)
			 	| (SYSCFG_IOCR0_GPEN9_SPI_GPIO54_H_MUX | SYSCFG_IOCR0_GPIO55_GPEN10_MUX(2) | SYSCFG_IOCR0_GPEN11_SPI_GPIO56_H_MUX)),
                		ADDR_SYSCFG_IOCR0);
			 writel((readl(ADDR_SYSCFG_IOCR2) |
				SYSCFG_IOCR2_GPIO53_GPEN8_MUX_H),
                		ADDR_SYSCFG_IOCR2);			 
		}
#endif
		break;
...
#define EXT_CTRL_REQ2	4
...
#if defined(CONFIG_LTN_DTV)
   			if (gpio_request(GPIO_GPS_EN_NUM, "gps_en")==0)
   			{
    				printk("GPIO_GPS_EN_NUM request granted\n");
    				gpio_direction_output(GPIO_GPS_EN_NUM, 0);
    				gpio_free(GPIO_GPS_EN_NUM);
   			}  
   			else
   			{
    				printk("GPIO_GPS_EN_NUM request error\n");
   			}  

			if (gpio_request(GPIO_GPS_RST_N, "gps_rst")==0)
			{
			   printk("GPIO_GPS_RST_N request granted\n"); 
			   gpio_direction_output(GPIO_GPS_RST_N, 0);
			   gpio_free(GPIO_GPS_RST_N);
			}
   			else
   			{
    				printk("GPIO_GPS_RST_N request error\n");
   			} 			
#endif
		}
		break;
	...
	case SYSCFG_CSL_DMA:
		...
		break;
      	case SYSCFG_TOUCH:
		if (op == SYSCFG_INIT) {
#if defined(CONFIG_LTN_DTV) // TSP_INT moved from GPIO30 to GPIO9
			/* No Pull GPIO9 */
			writel(readl(ADDR_GPIO_GPIPEN0) &
				(0xFFFFFDFF), ADDR_GPIO_GPIPEN0);
#else
			/* No Pull GPIO30 */
			writel(readl(ADDR_GPIO_GPIPEN0) &
...
#define GPIPUD_PULL_DOWN(GPIO) (~(1<<(GPIO%32)))
				...
				writel(readl(ADDR_GPIO_IOTR1)&(~(3<<IOTR_GPIO(30))),ADDR_GPIO_IOTR1);
#else
				...
				/*Enable pull up/down*/
#if defined(CONFIG_LTN_DTV) && defined(LTN_DTV_TSP_MODIFICATION)
				writel(readl(ADDR_GPIO_GPIPEN0)|GPIPEN_PULL_EN(26),ADDR_GPIO_GPIPEN0);
				writel(readl(ADDR_GPIO_GPIPEN0)|GPIPEN_PULL_EN(27),ADDR_GPIO_GPIPEN0);
				writel(readl(ADDR_GPIO_GPIPEN0)|GPIPEN_PULL_EN(30),ADDR_GPIO_GPIPEN0);
#else
				...
				/*Set as pull down*/
#if defined(CONFIG_LTN_DTV) && defined(LTN_DTV_TSP_MODIFICATION)
				writel(readl(ADDR_GPIO_GPIPUD0)&GPIPUD_PULL_DOWN(26),ADDR_GPIO_GPIPUD0);
				writel(readl(ADDR_GPIO_GPIPUD0)&GPIPUD_PULL_DOWN(27),ADDR_GPIO_GPIPUD0);
				writel(readl(ADDR_GPIO_GPIPUD0)&GPIPUD_PULL_DOWN(30),ADDR_GPIO_GPIPUD0);
#else
